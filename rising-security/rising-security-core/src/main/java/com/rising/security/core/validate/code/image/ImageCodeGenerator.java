 ...
 * 创建时间：2020-01-31 11:13:08
 * 创建作者：李启云
 */
@Data
public class ImageCodeGenerator implements ValidateCodeGenerator {

    public RisingSecurityProperties risingSecurityProperties;
    ...
    public ImageCode generate(ServletWebRequest request) {
        ...
        int width = ServletRequestUtils.getIntParameter(request.getRequest(), "width",
                risingSecurityProperties.getCode().getImage().getWidth());

        int height = ServletRequestUtils.getIntParameter(request.getRequest(), "height",
                risingSecurityProperties.getCode().getImage().getHeight());
        ...
        String sRand = "";
        for (int i = 0; i < risingSecurityProperties.getCode().getImage().getLength(); i++) {
        ...
        g.dispose();
        return new ImageCode(image, sRand, risingSecurityProperties.getCode().getImage().getExpireIn());
    }
