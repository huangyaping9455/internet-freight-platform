...
/**
 * 描述：继承InitializingBean并实现其afterPropertiesSet的作用是，
 * 在当前bean的其他参数都组装完成后，调用afterPropertiesSet初始化下面的urls变量
 * 创建时间：2020-01-31 11:54:37
 * 创建作者：李启云
 */
@Component("myValidateCodeFilter")
@Data
...
public class ValidateCodeFilter extends OncePerRequestFilter implements InitializingBean {
    ...
    public void afterPropertiesSet() throws ServletException {
        ...
        addUrlToMap(risingSecurityProperties.getCode().getImage().getUrls(), ValidateCodeType.IMAGE);

        urlMap.put(SecurityConstants.DEFAULT_LOGIN_PROCESSING_URL_MOBILE, ValidateCodeType.SMS);
        addUrlToMap(risingSecurityProperties.getCode().getSms().getUrls(), ValidateCodeType.SMS);
    }
